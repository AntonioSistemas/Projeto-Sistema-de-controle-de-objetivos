...
def chama_dados_conta():
    ...
    MENSAGEM_MAT_1.close()
    MENSAGEM_MAT_2.close()
...
def chama_cad_materia():
  ...
  # Recebendo Matéria
    ler_materia = ICAD_MATERIA.C.materia.text()
    verificar = "SELECT id FROM materia WHERE id_aluno='"+id_global+"' AND nome='"+ler_materia+"' "
    try:
        cursor=conexão.cursor()
        cursor.execute(verificar)
        mat=cursor.fetchall()
        str_materia=str(mat[0][0])
    
    except:
        print("erro")

    if str_materia == ler_materia:
        ICAD_MATERIA.close()
        MENSAGEM_MAT_1.show()
        
    else:   
        Adicionar_mat = "INSERT INTO materia (nome,id_aluno) VALUES('"+ler_materia+"'"+id_global+"')"
        cursor = conexão.cursor()
        cursor.execute(Adicionar_mat)
        conexão.commit()
        cursor.close()
...
def materia_existente():
    ...
    #botões
    MENSAGEM_MAT_1.B_MeusObjetivos.clicked.connect(chama_meus_objetivos)
    MENSAGEM_MAT_1.B_Conf_Conta.clicked.connect(chama_dados_conta)
    MENSAGEM_MAT_1.B_Sair.clicked.connect(chama_login)
    MENSAGEM_MAT_1.B_ok.clicked.connect(chama_perfil)
...
MENSAGEM_MAT_1 =uic.loadUi ("/home/sthefany/Documentos/projetosco/TELAS.PY/MENSAGEM_MAT_1.ui")
MENSAGEM_MAT_2 =uic.loadUi ("/home/sthefany/Documentos/projetosco/TELAS.PY/MENSAGEM_MAT_2.ui")
